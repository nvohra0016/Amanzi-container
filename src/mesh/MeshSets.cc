...
{
  AMANZI_ASSERT(ptype == Parallel_kind::ALL);
        ...
        const MeshCache<MemSpace_kind::HOST>& mesh)
{
  ...
  if (ptype == Parallel_kind::OWNED) {
    ...
    ents = Kokkos::subview(ents, Kokkos::make_pair((size_type)0, (size_type)mesh.getNumEntities(boundary_kind, ptype)));
  }
                         ...
                         const MeshCache<MemSpace_kind::HOST>& mesh)
{
  if (kind == createEntityKind(region.entity_str())) {
    ...
    View_type<Entity_ID, MemSpace_kind::HOST> mesh_ents("mesh_ents", mesh_map->getLocalNumElements());
