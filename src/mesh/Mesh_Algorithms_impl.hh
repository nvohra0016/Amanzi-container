...
getFaceOnBoundaryInternalCell(const Mesh_type& mesh, Entity_ID f)
{
  if constexpr(std::is_same<Mesh_type, Mesh>::value) {
    assert(mesh.getFaceNumCells(f, Parallel_kind::ALL) == 1 && "getFaceOnBoundaryInternalCell() requires a face on the boundary.");
  } else {
...
{
  boundary_faces.doImport(faces, mesh.getBoundaryFaceImporter(), Tpetra::INSERT);
}
...
AmanziGeometry::Point
MeshAlgorithms::computeFaceNormal(
  const MeshHost& mesh, const Entity_ID f, const Entity_ID c, int * const orientation) const {
...
AmanziGeometry::Point
MeshAlgorithms::computeEdgeVector(const MeshHost& mesh, const Entity_ID e, const Entity_ID n, int * const orientation) const {
  auto geom = Impl::computeEdgeGeometry(mesh, e);
  if (n >= 0) {
    auto nodes = mesh.getEdgeNodes(e);
    if (n == nodes[0]) {
...
void
MeshAlgorithms::computeCellFacesAndBisectors(const MeshHost& mesh, const Entity_ID cellid,
        typename MeshHost::cEntity_ID_View& faceids, typename MeshHost::cPoint_View * const bisectors) const {
  mesh.getCellFaces(cellid, faceids);
  if (bisectors)
    *bisectors = Impl::computeBisectors(mesh, cellid, faceids);
}
...
createMapsFromMeshGIDs(const Mesh_type& mesh, const Entity_kind kind)
{
  ...
  auto gids_owned = Kokkos::subview(gids, Kokkos::make_pair((std::size_t)0, (std::size_t)num_owned));
...
createContiguousMaps(const Map_ptr_type& ghosted, const Map_ptr_type& owned)
{
  ...
  auto owned_contiguous = Teuchos::rcp(new Map_type(owned->getGlobalNumElements(),
          owned->getLocalNumElements(), 0, owned->getComm()));
  ...
  Vector_type_<GO> owned_contiguous_vec(owned);
  {
    ...
    for (int i=0; i!=data.size(); ++i) {
      data(i,0) = owned_contiguous->getGlobalElement(i);
    }
  }
  ...
  auto ghosted_contiguous = Teuchos::rcp(new Map_type(-1, all_contiguous_vec.getData()(),
          0, ghosted->getComm()));
