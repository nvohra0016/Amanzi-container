...
void MeshEmbeddedLogicalAlgorithms::computeCellFacesAndBisectors(
          const MeshHost& mesh, 
          const Entity_ID cellid,
          MeshFramework::cEntity_ID_View& faceids,
          MeshFramework::cPoint_View * const bisectors) const 
{
  static_cast<const MeshEmbeddedLogical*>(mesh.getMeshFramework().get())->getCellFacesAndBisectors(cellid, faceids, bisectors);
}

double MeshEmbeddedLogicalAlgorithms::computeCellVolume(const MeshHost& mesh, const Entity_ID c) const 
{
  return static_cast<const MeshEmbeddedLogical*>(mesh.getMeshFramework().get())->getCellVolume(c); 
}

AmanziGeometry::Point MeshEmbeddedLogicalAlgorithms::computeCellCentroid(const MeshHost& mesh, const Entity_ID c) const
{
  return static_cast<const MeshEmbeddedLogical*>(mesh.getMeshFramework().get())->getCellCentroid(c); 
}

double MeshEmbeddedLogicalAlgorithms::computeFaceArea(const MeshHost& mesh, const Entity_ID f) const
{
  return static_cast<const MeshEmbeddedLogical*>(mesh.getMeshFramework().get())->getFaceArea(f); 
}

AmanziGeometry::Point MeshEmbeddedLogicalAlgorithms::computeFaceCentroid(const MeshHost& mesh, const Entity_ID f) const
{
  return static_cast<const MeshEmbeddedLogical*>(mesh.getMeshFramework().get())->getFaceCentroid(f); 
}

AmanziGeometry::Point MeshEmbeddedLogicalAlgorithms::computeFaceNormal(const MeshHost& mesh, const Entity_ID f,
...
void
MeshEmbeddedLogical::getFaceNodes(const Entity_ID f, MeshFramework::cEntity_ID_View& nodes) const
{
  Errors::Message mesg("There are no nodes in a MeshEmbeddedLogical.");
  Exceptions::amanzi_throw(mesg);
}
...
MeshEmbeddedLogical::getNodeFaces(const Entity_ID nodeid,
                  ...
                  MeshFramework::cEntity_ID_View& faceids) const
{
  Errors::Message mesg("There are no nodes in a MeshEmbeddedLogical.");
  Exceptions::amanzi_throw(mesg);
}
...
MeshEmbeddedLogical::getCellFacesAndDirs(
  ...
  MeshFramework::cDirection_View * const dirs) const
{
  MeshFramework::Entity_ID_View lfaces; 
  MeshFramework::Direction_View ldirs; 
...
MeshEmbeddedLogical::getFaceCells(const Entity_ID f,
        const Parallel_kind ptype,
        MeshFramework::cEntity_ID_View& cells) const
{
  MeshFramework::Entity_ID_View lcells; 
  auto ncells_log = log_mesh_->getNumEntities(Entity_kind::CELL, Parallel_kind::OWNED);
  auto nfaces_log = log_mesh_->getNumEntities(Entity_kind::FACE, Parallel_kind::OWNED);
  auto nfaces_extra = extra_face_cell_ids_.size<MemSpace_kind::HOST>();
  if (f < nfaces_log) {
