...
TEST(MESH_SURFACE_EXTRACTION_GENERATED)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    // make a MeshCache
    auto mesh = Teuchos::rcp(new Mesh(surface_framework_mesh, Teuchos::rcp(new AmanziMesh::MeshAlgorithms()), Teuchos::null));
...
TEST(MESH_SURFACE_EXTRACTION_GENERATED_EXTRACTED_MANIFOLD)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    auto parent_mesh_cache = Teuchos::rcp(new Mesh(parent_mesh, Teuchos::rcp(new AmanziMesh::MeshAlgorithms()), Teuchos::null)); 
