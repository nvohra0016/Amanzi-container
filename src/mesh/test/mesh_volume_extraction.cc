...
TEST(MESH_VOLUME_EXTRACTION_GENERATED)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    // make a MeshCache
    auto mesh = Teuchos::rcp(new Mesh(vol_framework_mesh, Teuchos::rcp(new AmanziMesh::MeshAlgorithms()), Teuchos::null));
...
TEST(MESH_VOLUME_EXTRACTION_EXO)
{
  ...
  for (const auto& frm : frameworks) {
    ...
    auto column_mesh = Teuchos::rcp(new Mesh(column_mesh_fw, Teuchos::rcp(new AmanziMesh::MeshAlgorithms()), Teuchos::null));
