        ...
        const Parallel_kind ptype) const
{
  ...
  if (check_new) {
    ...
    int gsize = 0;
    Teuchos::reduceAll(*getComm(), Teuchos::REDUCE_SUM, 1, &lsize, &gsize);
    if (gsize == 0) {
      Errors::Message msg;
...
{
  auto cf = ComputeFunction<MEM>::template hostOnly<std::function<AmanziGeometry::Point(const Entity_ID)>>();
  if constexpr(MEM == MemSpace_kind::HOST) {
...
template<AccessPattern_kind AP>
KOKKOS_INLINE_FUNCTION
typename MeshCache<MEM>::cPoint_View
...
template<MemSpace_kind MEM>
void MeshCache<MEM>::setNodeCoordinate(const Entity_ID n,
                         const AmanziGeometry::Point& p)
{
  if (data_.node_coordinates_cached)
    view<MemSpace_kind::HOST>(data_.node_coordinates)(n) = p;
  if (framework_mesh_.get())
    framework_mesh_->setNodeCoordinate(n, p);
}

template<MemSpace_kind MEM>
void MeshCache<MEM>::setNodeCoordinates(const cEntity_ID_View& nodes,
                        const cPoint_View& new_coords)
{
  ...
  if (data_.node_coordinates_cached) {
    if constexpr(MEM == MemSpace_kind::HOST) {
      ...
      Kokkos::parallel_for("setNodeCoordinates", nodes.size(),
...
template<AccessPattern_kind AP>
KOKKOS_INLINE_FUNCTION
Entity_ID MeshCache<MEM>::getEdgeNode(const Entity_ID e, const size_type i) const{
  // Compute list and use only one?
  cEntity_ID_View nodes;
...
template<AccessPattern_kind AP>
KOKKOS_INLINE_FUNCTION
AmanziGeometry::Point MeshCache<MEM>::getEdgeCentroid(const Entity_ID c) const {
  auto cf = ComputeFunction<MEM>::template hostOnly<std::function<AmanziGeometry::Point(const Entity_ID)>>();
  if constexpr(MEM == MemSpace_kind::HOST) {
...
template<AccessPattern_kind AP>
KOKKOS_INLINE_FUNCTION
AmanziGeometry::Point MeshCache<MEM>::getEdgeVector(const Entity_ID e) const {
  auto cf = ComputeFunction<MEM>::template hostOnly<std::function<AmanziGeometry::Point(const Entity_ID)>>();
  if constexpr(MEM == MemSpace_kind::HOST) {
...
void MeshCache<MEM>::cacheCellGeometry()
{
  ...
  } catch(Errors::FrameworkNotImplemented& e) {}
}
