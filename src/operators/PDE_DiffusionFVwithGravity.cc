    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int c) {
        ...
        for (int i = 0; i < faces.extent(0); i++) {
          ...
          // if (c == 0) std::cout << "  1/trans_f(" << f << ") += " << dxn <<
          // "/" << perm << " = " << fabs(dxn/perm) << std::endl;
...

  {
    ...
    Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int f) {
        ...
        if (ncells == 2) {
          a_dist = m->getCellCentroid(cells[1]) - m->cell_centroid(cells[0])
        } else if (ncells == 1) {
          a_dist = m->face_centroid(f) - m->getCellCentroid(cells[0])
        }
        ...
        // if (f == 0) std::cout << "grav_f = " << trans_f(f,0) << " * " <<
        // h_f(f,0) << " * g * " << a_dist << " * " << rho << std::endl;
