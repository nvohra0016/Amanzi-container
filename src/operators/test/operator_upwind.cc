...
#ifdef HAVE_MPI
    double tmp = error;
    Teuchos::reduceAll<int>(*mesh->get_comm(),Teuchos::REDUCE_SUM, 1,&tmp, &error);
    int itmp = nfaces_owned;
    Teuchos::reduceAll<int>(*mesh->get_comm(),Teuchos::REDUCE_SUM, 1,&itmp, &nfaces_owned);
#endif
    error = sqrt(error / nfaces_owned);

    if (comm->getRank() == 0) printf("n=%2d %s=%8.4f\n", n, method.c_str(), error);
  }
}
