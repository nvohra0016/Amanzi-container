...
/*
  ...
  RegisteredFactory<MFD3D_MyMethod> MFD3D_MyMethod::factory_("my unique
  method");
*/
...
class MFD3D_Diffusion_Edge : public MFD3D {
 public:
  ...
  virtual int
  L2consistency(int c, constTensor<>& T, DenseMatrix<>& N, DenseMatrix<>& Mc, bool symmetry) override
  {
    return -1;
  }
  virtual int MassMatrix(int c, constTensor<>& T, DenseMatrix<>& M) override { return -1; }
  ...
  virtual int MassMatrixInverse(int c, constTensor<>& K, DenseMatrix<>& W) override { return -1; }

  // -- stiffness matrix
  virtual int H1consistency(int c, constTensor<>& K, DenseMatrix<>& N, DenseMatrix<>& Ac) override;
  virtual int StiffnessMatrix(int c, constTensor<>& K, DenseMatrix<>& A) override;
