...
FaceNormalExterior(const AmanziMesh::Mesh& mesh, int f, bool* flag)
{
  ...
  mesh.face_get_cells_host(f, AmanziMesh::Parallel_kind::ALL, cells);
  *flag = (cells.extent(0) == 1);

  int dir;
  AmanziGeometry::Point normal;
  if (*flag) normal = mesh.face_normal_host(f, false, cells[0], &dir);
...
GlobalOp(const Comm_type& comm, std::string op, double* val, int n)
{
  ...
  if (op == "sum")
    Teuchos::reduceAll(comm, Teuchos::REDUCE_SUM, *val_tmp, Teuchos::outArg(*val));
  else if (op == "max")
    Teuchos::reduceAll(comm, Teuchos::REDUCE_MAX, *val_tmp, Teuchos::outArg(*val));
  ...
  for (int c = 0; c < ncells; c++) {
    ...
    for (int k = 0; k < nnodes; k++) {
      ...
      if (std::abs(tmp - p(v, 0)) > .01) {
        ...
        xv2 = mesh->getNodeCoordinate(v);
        // std::cout << v << " at " << xv << " error: " << tmp << " " << p(v,0)
        // << std::endl;
      }
                        ...
                        double& inf_err)
{
  ...
  auto p = p_vec.ViewComponent<MirrorHost>(
    "edge_moments", false); // likely this will fail, not sure what it sould be.
                            // fix when it fails.
  for (int c = 0; c < ncells; c++) {
    double volume = mesh->cell_volume_host(c);
