...
* Initialization
****************************************************************** */
void PDE_DiffusionFVonManifolds::Init_(Teuchos::ParameterList& plist)
{
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::SetTensorCoefficient(
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::SetScalarCoefficient(
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::UpdateMatrices(
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::ApplyBCs(bool primary, bool eliminate, bool essential_eqn)
{
  const auto& beta_f = *beta_->ViewComponent("face", true);
  const auto& fmap = *beta_->Map().Map("face", true);
  ...
  for (int f = 0; f < nfaces_owned; f++) {
    if (bc_model[f] != OPERATOR_BC_NONE) {
      ...
      if (bc_model[f] == OPERATOR_BC_DIRICHLET && primary) {
        ...
        if (primary)
          rhs_c[0][c] -= bc_value[f] * mesh_->face_area(f);
      }
    }
  }
}
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::UpdateFlux(
...
****************************************************************** */
void PDE_DiffusionFVonManifolds::ComputeBeta_()
{
