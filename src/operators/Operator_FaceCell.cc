                                     ...
                                     CompositeVector& Y) const
{
  ...
  Kokkos::parallel_for(
    ...
    KOKKOS_LAMBDA(const int& sc) {
      auto f = mesh->getEntityParent(AmanziMesh::Entity_kind::CELL, sc);
      // atomic not needed
                                     ...
                                     CompositeVector& Y) const
{
  ...
  Kokkos::parallel_for(
    ...
    KOKKOS_LAMBDA(const int& sf) {
      ...
      mesh->face_get_cells(sf, AmanziMesh::Parallel_kind::ALL, cells);
      ...
      for (int n = 0; n != ncells; ++n) {
        cells(n) = mesh->getEntityParent(AmanziMesh::Entity_kind::CELL, cells(n));
        lv(n) = Xf(cells(n), 0);
      }
      lA.Multiply(lv, lAv, false);

      for (int n = 0; n != ncells; ++n) { Kokkos::atomic_add(&Yf(cells(n), 0), lAv(n)); }
    });
  return 0;
}
                                            ...
                                            int my_block_col) const
{
  ...
  for (int sf = 0; sf != nsurf_faces; ++sf) {
    AmanziMesh::Entity_ID_View cells;
    op.mesh->face_get_cells(sf, AmanziMesh::Parallel_kind::ALL, cells);
      ...
      for (int n = 0; n != cells.extent(0); ++n) {
        ...
        if (face_row_inds[cells[n]] < nrows_local) {
          for (int m = 0; m != cells.extent(0); ++m) {
            proc_mat.sumIntoValues(
              face_row_inds(cells(n)), &face_col_inds(cells(m)), 1, &A_f(n, m), true, true);
          }
        } else {
          for (int m = 0; m != cells.extent(0); ++m) {
            offproc_mat.sumIntoValues(face_row_inds(cells(n)) - nrows_local,
                                      &face_col_inds(cells(m)),
                                      1,
                                      &A_f(n, m),
                                      true,
                                      true);
