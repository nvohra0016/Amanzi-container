...
TreeOperator::initializeInverse()
{
  ...
  if (inv_plist_.isParameter("preconditioning method") &&
    ...
    // indices, which need structure.  Since not guaranteed structure until
    // Initialize, is called, we cannot set block indicies until now. provide
    // block ids for block strategies.
    if (!row_supermap_.get()) row_supermap_ = createSuperMap(*get_row_map());
  ...
  if (inv_plist_.isParameter("preconditioning method") &&
    ...
    // method on top, or a direct method.  Call the factory, which assumes this
    // is an assembler factory.
