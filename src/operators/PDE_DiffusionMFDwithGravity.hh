...
GravitySpecialDirection(const AmanziMesh::Mesh* const mesh, int f)
{
  ...
  if (ncells == 2) {
    return mesh->getCellCentroid(cells[1]) - mesh->cell_centroid(cells[0])
  } else {
    return mesh->face_centroid(f) - mesh->getCellCentroid(cells[0])
  }
}
} // namespace Impl
...
class PDE_DiffusionMFDwithGravity : public PDE_DiffusionMFD {
 public:
  ...
  // virtual void UpdateFluxNonManifold(const Teuchos::Ptr<const
  // CompositeVector>& u,
  //                                    const Teuchos::Ptr<CompositeVector>&
  //                                    flux) override;
