...
Operator::SymbolicAssembleMatrix()
{
  ...
  Amat_->fillComplete(); // some inverse methods seem to need this fillComplete,
                         // even with no data.
  A_ = Amat_->getMatrix();
}
...
//                                         int my_block_row, int my_block_col)
//                                         const
// {
//   AMANZI_ASSERT(cvs_col_->hasComponent(op.col_compname()));
//   AMANZI_ASSERT(cvs_row_->hasComponent(op.row_compname()));

//   const std::vector<int>& row_gids = map.GhostIndices(my_block_row,
//   op.row_compname(), 0); const std::vector<int>& col_gids =
...
//     }
//     ierr |= graph.insertLocalIndices(ndofs, lid_r.data(), ndofs,
//     lid_c.data());
//   }
//   AMANZI_ASSERT(!ierr);
// }
...
void
Operator::WriteMatrix(const std::string& fname_base)
{
  // write to disk
          ...
          while (smap_->hasComponent(lcv_block, comp, lcv_dof)) {
            ...
            for (int i = 0; i != ids.extent(0); ++i) {
              os << r << " " << comp << " " << i << " " << mesh_map->getGlobalElement(i) << " "
                 ...
                 << AmanziMesh::get_coordinate(*mesh_, AmanziMesh::entity_kind(comp), i)
                 << std::endl;
            }
            lcv_dof++;
          }
...
//     ierr |= mat.sumIntoLocalValues(lid_r.data(), lid_c.data(),
//     op.matrices[n]);
//   }
//   AMANZI_ASSERT(!ierr);
// }
...
//                                 const CompositeVector& X, CompositeVector& Y)
//                                 const
// {
//   AMANZI_ASSERT(cvs_col_->hasComponent(op.col_compname()));
//   AMANZI_ASSERT(cvs_row_->hasComponent(op.row_compname()));
...
{
  if (!volume_included) {
    Errors::Message msg("Operator::UpdateRHS expected volume_included == True "
...
//                                    WhetStone::DenseVector<>& v, const
//                                    CompositeVector& X) const {
//   Errors::Message msg("Extracton fo local cell-based vector is missing for
//   this operator"); Exceptions::amanzi_throw(msg);
// }
...
//                                     const WhetStone::DenseVector<>& v,
//                                     CompositeVector& X) const {
//   Errors::Message msg("Assembly fo local cell-based vector is missing for
//   this operator"); Exceptions::amanzi_throw(msg);
// }
...
//                                    WhetStone::DenseVector<>& v, const
//                                    CompositeVector& X) const {
//   Errors::Message msg("Extracton fo local node-based vector is missing for
//   this operator"); Exceptions::amanzi_throw(msg);
// }
...
//                                     const WhetStone::DenseVector<>& v,
//                                     CompositeVector& X) const {
//   Errors::Message msg("Assembly fo local node-based vector is missing for
//   this operator"); Exceptions::amanzi_throw(msg);
// }
