  ...
  Kokkos::parallel_for(
    "PDE_DiffusionMFD::UpdateMatricesMixed_little_k_", ncells_owned, KOKKOS_LAMBDA(const int& c) {
      ...
      if (little_k_type_ == OPERATOR_LITTLE_K_DIVK) {
        ...
        // -- same as above but remains second-order for dicontinuous
        // coefficients
...
PDE_DiffusionMFD::UpdateMatricesNodal_()
{
  ...
  //    Errors::Message msg("Stiffness_MFD: unexpected failure of LAPACK in
  //    WhetStone."); Exceptions::amanzi_throw(msg);
  //  }
...
PDE_DiffusionMFD::UpdateMatricesTPFA_()
{
  ...
  // Teuchos::RCP<CompositeVector> T = cv_space.Create();
  // //Teuchos::RCP<CompositeVector>(new CompositeVector(cv_space, true)); const
  ...
  //    WhetStone::DenseMatrix<DeviceOnlyMemorySpace>
  //    Mff(Mffv.at(c),Mffv.size(c,0),Mffv.size(c,1));
  ...
  //     WhetStone::DenseMatrix<DeviceOnlyMemorySpace>
  //     Aface(A.at(f),A.size(f,0),A.size(f,1));
...
PDE_DiffusionMFD::ApplyBCs(bool primary, bool eliminate, bool essential_eqn)
{
  ...
  if (!exclude_primary_terms_) {
    if (local_op_schema_ ==
      ...
      // AMANZI_ASSERT(bcs_trial_.size() == 1);
      // AMANZI_ASSERT(bcs_test_.size() == 1);
      // ApplyBCs_Cell_(bcs_trial_[0].ptr(), bcs_test_[0].ptr(), primary,
      // eliminate, essential_eqn);
                                           ...
                                           const Teuchos::Ptr<const CompositeVector>& factor)
{
  ...
  //  int i, dir, c1;
  //  c1 = cells[0];
  //  const AmanziGeometry::Point& normal = mesh_->face_normal(f, false, c1,
  //  &dir); i = (v * dir >= 0.0) ? 0 : 1;
...
PDE_DiffusionMFD::Init()
{
  ...
  if (global_op_ == Teuchos::null) {
    ...
    if (schema_prec_dofs_ == OPERATOR_SCHEMA_DOFS_NODE) {
      ...
      // global_op_ = Teuchos::rcp(new Operator_Cell(cvs->CreateSpace(), plist_,
      // schema_prec_dofs_));
  ...
  if (!exclude_primary_terms_) {
    if (local_op_schema_ == (OPERATOR_SCHEMA_BASE_CELL | OPERATOR_SCHEMA_DOFS_NODE)) {
      ...
      if (plist_.get<bool>("surface operator", false)) {
        ...
        // local_op_ = Teuchos::rcp(new Op_SurfaceFace_SurfaceCell(name,
        // mesh_));
      } else {
        assert(false);
  ...
  if (jacobian == "none") {
    ...
    if (!(schema_prec_dofs_ & OPERATOR_SCHEMA_DOFS_CELL)) {
      Errors::Message msg("PDE_DiffusionMFD: incompatible options.  \"approximate Jacobian\" "
...
PDE_DiffusionMFD::UpdateConsistentFaces(CompositeVector& u)
{
  ...
  //  create the op
  //  Teuchos::RCP<CompositeVectorSpace> cface_cvs = Teuchos::rcp(new
  //  CompositeVectorSpace()); cface_cvs->SetMesh(mesh_)->SetGhosted()
  //      ->AddComponent("face", AmanziMesh::Entity_kind::FACE, 1);

  //  consistent_face_op_ = Teuchos::rcp(new Operator_ConsistentFace(cface_cvs,
  //  plist_.sublist("consistent faces")));
  //  consistent_face_op_->OpPushBack(local_op_);
  //  consistent_face_op_->SymbolicAssembleMatrix();
  //  consistent_face_op_->InitializePreconditioner(plist_.sublist("consistent
  //  faces").sublist("preconditioner"));
  //}
  ...
  //   AmanziSolvers::LinearOperatorFactory<Operator, CompositeVector,
  //   CompositeVectorSpace> fac; Teuchos::RCP<Operator> lin_solver =
  //   fac.Create(
  //       plist_.sublist("consistent faces").sublist("linear solver"),
  //       consistent_face_op_);
...
PDE_DiffusionMFD::ComputeTransmissibility(int f) const
{
  ...
  mesh_->face_get_cells_host(f, AmanziMesh::Parallel_kind::ALL, cells);
  int c = cells[0];
