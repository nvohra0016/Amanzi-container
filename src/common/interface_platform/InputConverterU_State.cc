...
****************************************************************** */
void InputConverterU::TranslateCommonContinuumFields_(
    ...
    Teuchos::ParameterList& out_ev) 
{
  ...
  if (flag) {
    ...
    AddIndependentFieldEvaluator_(out_ev, Keys::getKey(domain, "viscosity_gas"),
                                  "All", "cell", viscosity);
  }
  ...
  for (int i = 0; i < children->getLength(); i++) {
    ...
    if (DOMNode::ELEMENT_NODE == inode->getNodeType()) {
      ...
      if (flag) TranslateFieldIC_(node, Keys::getKey(domain, "specific_storage"),
                                  "m^-1", reg_str, regions, out_ic);

      // specific_yield
      node = GetUniqueElementByTagsString_(inode, "mechanical_properties, specific_yield", flag);
      if (flag) TranslateFieldIC_(node, Keys::getKey(domain, "specific_yield"),
                                  "-", reg_str, regions, out_ic);

      // particle density
      node = GetUniqueElementByTagsString_(inode, "mechanical_properties, particle_density", flag);
      if (flag) TranslateFieldEvaluator_(node, Keys::getKey(domain, "particle_density"),
                                         "kg*m^-3", reg_str, regions, out_ic, out_ev, type, domain);
