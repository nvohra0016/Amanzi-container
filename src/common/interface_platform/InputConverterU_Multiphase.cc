    ...
    const std::string& domain, Teuchos::ParameterList& state_list)
{
  ...
  fev.sublist(mol_density_liquid_key)
     .set<std::string>("pressure key", pressure_liquid_key);
  ...
  auto evals = Teuchos::Array<std::string>(
    { ncp_g_key, storage_water_key, storage_tcc_key,
      adv_water_key, adv_liquid_key, adv_gas_key,
      diff_liquid_key, diff_gas_key, diff_vapor_key,
      sat_gas_key, mole_xl_key, mole_xv_key });
  ...
  peqn.set<std::string>("primary unknown", pl)
      .set<Teuchos::Array<std::string> >("advection liquid", std::vector<std::string>({ adv_water_key, pl }))
      .set<Teuchos::Array<double> >("advection factors", ones)
      .set<Teuchos::Array<std::string> >("diffusion liquid", std::vector<std::string>({ diff_vapor_key, mole_xv_key }))
  ...
  seqn.set<std::string>("primary unknown", xg)
      .set<Teuchos::Array<std::string> >("advection liquid", std::vector<std::string>({ adv_liquid_key, pl }))
      .set<Teuchos::Array<std::string> >("advection gas", std::vector<std::string>({ adv_gas_key, pressure_gas_key }))
      .set<Teuchos::Array<double> >("advection factors", ones)
      .set<Teuchos::Array<std::string> >("diffusion liquid", std::vector<std::string>({ diff_liquid_key, mole_xl_key }))
      .set<Teuchos::Array<std::string> >("diffusion gas", std::vector<std::string>({ diff_gas_key, mole_xg_key }))
      .set<Teuchos::Array<double> >("diffusion factors", ones)
  ...
  ceqn.set<std::string>("primary unknown", sat_liquid_key)
      .set<Teuchos::Array<std::string> >("ncp evaluators", std::vector<std::string>({ ncp_f_key, ncp_g_key }));
  ...
  fev.sublist(adv_water_key)
     ...
     .set<Teuchos::Array<std::string> >("dependencies", 
        std::vector<std::string>({ mol_density_liquid_key, relperm_liquid_key, viscosity_liquid_key }))
  ...
  fev.sublist(adv_liquid_key)
     .set<std::string>("evaluator type", "product")
     .set<Teuchos::Array<std::string> >("dependencies", 
         std::vector<std::string>({ mol_density_liquid_key, mole_xl_key,
                                    relperm_liquid_key, viscosity_liquid_key }))
  ...
  fev.sublist(adv_gas_key)
     .set<std::string>("evaluator type", "product")
     .set<Teuchos::Array<std::string> >("dependencies", 
         std::vector<std::string>({ mol_density_gas_key, mole_xg_key,
                                    relperm_gas_key, viscosity_gas_key }))
     .set<Teuchos::Array<int> >("powers", std::vector<int>({ 1, 1, 1, -1 }))
     .set<std::string>("tag", "");
  ...
  fev.sublist(mole_xl_key)
   ...
   fev.sublist(mole_xv_key)
     .set<std::string>("evaluator type", "product")
     .set<Teuchos::Array<std::string> >("dependencies", std::vector<std::string>({ pressure_gas_key, pressure_vapor_key }))
